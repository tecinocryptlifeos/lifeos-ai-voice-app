...
The production Cloudflare Worker must never depend on `NORTHFLANK_ORIGIN` being populated. A missing or empty optional standby is a valid steady-state configuration, not a deployment error. The scheduled health path must remain edge-authoritative and must not throw because a standby binding is absent.
